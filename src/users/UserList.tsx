...
import { getInitials, imgSrc } from './UserProfileAvatarInput';

const NAV_TEAL = '#3d7a7a';
const NAV_TEAL_DARK = '#2e6262';

const USER_COLUMNS_STORE_KEY = 'users.columns';
const USER_COLUMNS_BUTTON_ID = 'users.columnsButton';

function UserListActions() {
    const navigate = useNavigate();
    const { page, perPage, total, setPage } = useListContext();
    const [q, setQ] = useOdooListSearchQ();
    const [view, setView] = useStore<'list' | 'kanban'>('users.listView', 'list');
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px', flex: '0 0 auto' }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, ml: 0.5, position: 'relative' }}>
                        ...
                        <IconButton
                            ...
                            onClick={() => {
                                const el = document.getElementById(USER_COLUMNS_BUTTON_ID) as HTMLButtonElement | null;
                                el?.click();
                            }}
                        >
                        ...
                        >
                            <ColumnsButton
                                id={USER_COLUMNS_BUTTON_ID}
                                storeKey={USER_COLUMNS_STORE_KEY}
...
function UserKanban() {
    ...
    return (
        <Box sx={{ p: 1.5 }}>
            ...
            >
                {rows.map((r: { id?: string; fullName?: string; email?: string; role?: string; isActive?: boolean; profileImage?: string }) => {
                    const name = r?.fullName ?? r?.id ?? 'User';
                    const src = imgSrc(r?.profileImage);
                    ...
                    return (
                        <Paper
                            ...
                            onClick={() => r.id && navigate(`/users/${encodeURIComponent(r.id)}`)}
                        >
                            <Box
                                sx={{
                                    width: '100%',
...
export default function UserList() {
    const [view] = useStore<'list' | 'kanban'>('users.listView', 'list');
        ...
        >
            {view === 'kanban' ? (
                ...
                <DataTable
                    ...
                    storeKey={USER_COLUMNS_STORE_KEY}
                    sx={{
                        '& .column-profileImage': { width: 64 },
                        '& .MuiTableCell-head': { fontWeight: 700 },
                    }}
                >
                    <Column source="profileImage" label="Photo" disableSort>
                        <FunctionField
                            label=""
                            source="profileImage"
                            render={(record: { profileImage?: string; fullName?: string }) => {
                                const src = imgSrc(record?.profileImage);
                                return src ? (
                                    <Box
                                        component="img"
                                        src={src}
