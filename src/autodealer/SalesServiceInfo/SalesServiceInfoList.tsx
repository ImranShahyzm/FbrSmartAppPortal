...
function SalesServiceInformationListActions() {
    ...
    const [view, setView] = useStore<'list' | 'kanban'>('salesServiceInfo.listView', 'list');
            ...
            <Box sx={{ width: '100%', bgcolor: 'common.white', borderBottom: '1px solid', borderColor: 'divider', px: 2, py: '6px', display: 'flex', alignItems: 'center', gap: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px', flex: '0 0 auto' }}>
                    <Button
                        variant="contained"
                        size="small"
                        onClick={() => navigate(createPath({ type: 'create', resource: 'salesServiceInfo' }))}
                        sx={{ 
                            bgcolor: NAV_TEAL, 
                            color: '#fff', 
                            textTransform: 'none', 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, ml: 0.5 }}>
                        ...
                        <IconButton 
                            ...
                            onClick={() => document.getElementById(SALES_SERVICE_COLUMNS_BUTTON_ID)?.click()}
                        >
                            <SettingsOutlinedIcon sx={{ fontSize: 16 }} />
                        </IconButton>
                        <Box sx={{ display: 'none' }}>
                            <ColumnsButton id={SALES_SERVICE_COLUMNS_BUTTON_ID} storeKey={SALES_SERVICE_COLUMNS_STORE_KEY} />
                        </Box>
                    </Box>
                </Box>
...
function SalesServiceInformationKanban() {
    ...
    return (
        ...
        >
            {rows.map((record: any) => {
                const id = record.saleServiceInfoID || record.SaleServiceInfoID || record.saleServiceID || record.SaleServiceID || record.id;
...
export default function SalesServiceInformationList() {
    const [view] = useStore<'list' | 'kanban'>('salesServiceInfo.listView', 'list');
    ...
    return (
        <List
            ...
            perPage={25}
            sort={{ field: 'saleServiceName', order: 'ASC' }}   // Better default sort
        >
            {view === 'kanban' ? (
                <SalesServiceInformationKanban />
            ) : (
                <DataTable
                    rowClick="edit"
                ...
                >
                    <Column source="saleServiceInfoID" label="ID" />           {/* Primary Key */}
                    <Column source="saleServiceName" label="Sale Service Name" />
                    <Column source="saleServiceAccountNumber" label="Account Number" />
