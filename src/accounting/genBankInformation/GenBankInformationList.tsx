...
const GEN_BANK_COLUMNS_STORE_KEY = 'genBankInformation.columns';
const GEN_BANK_COLUMNS_BUTTON_ID = 'genBankInformation.columnsButton';
...
function EmptyBankInformation() {
    const translate = useTranslate();
    return (
        <Box sx={{ py: 4, px: 2, textAlign: 'center', maxWidth: 420, mx: 'auto' }}>
            <Typography variant="body1" color="text.secondary" paragraph>
                {translate('shell.accounting.gen_bank_information_empty', {
                    _: 'No bank accounts yet. Create one to link postings to a bank GL account.',
                })}
            </Typography>
        </Box>
    );
...
function BankInformationListToolbar() {
    const translate = useTranslate();
    ...
    const canCreate = useAccountingAccess('genBankInformation', 'create');
            ...
            >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px', flex: '0 0 auto' }}>
                    {canCreate ? (
                        <Button
                            component={Link}
                            to={createPath({ resource: 'genBankInformation', type: 'create' })}
                            variant="contained"
                            size="small"
                            sx={{
                                bgcolor: NAV_TEAL,
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, ml: 0.5 }}>
                        <Typography variant="body1" sx={{ fontWeight: 700, fontSize: 15, color: 'text.primary' }}>
                            {translate('resources.genBankInformation.name', { smart_count: 2 })}
                        </Typography>
                        <IconButton
                            size="small"
                        ...
                        >
                            <ColumnsButton
                                id={GEN_BANK_COLUMNS_BUTTON_ID}
                                storeKey={GEN_BANK_COLUMNS_STORE_KEY}
                ...
                <Box sx={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
                    ...
                    >
                        ...
                        <Typography variant="body2" color="text.secondary">
                            {translate('shell.accounting.gen_bank_filters_placeholder', {
                                _: 'Use the search box to find banks by title or details. More filters can be added later.',
                            })}
                        </Typography>
                    </Popover>
                </Box>
...
function BankBulkActions() {
    const canDelete = useAccountingAccess('genBankInformation', 'delete');
    if (!canDelete) return false;
    return <BulkDeleteButton />;
}

export function GenBankInformationList() {
    const translate = useTranslate();
    ...
    return (
        <List
            ...
            perPage={25}
            actions={<BankInformationListToolbar />}
            empty={<EmptyBankInformation />}
            title={<Title />}
            sx={{
        ...
        >
            <DataTable
                rowClick="edit"
                bulkActionButtons={<BankBulkActions />}
                storeKey={GEN_BANK_COLUMNS_STORE_KEY}
                sx={{
                    '& .MuiTableCell-head': {
                        bgcolor: 'grey.100',
            ...
            >
                <Column source="bankAccountTitle" label={translate('resources.genBankInformation.fields.bank_account_title')}>
                    <TextField source="bankAccountTitle" />
                </Column>
                <Column source="bankName" label={translate('resources.genBankInformation.fields.bank_name')}>
                    <TextField source="bankName" emptyText="—" />
                </Column>
                <Column source="bankAccountNumber" label={translate('resources.genBankInformation.fields.bank_account_number')}>
                    <TextField source="bankAccountNumber" emptyText="—" />
                </Column>
                <Column source="bankBranchCode" label={translate('resources.genBankInformation.fields.bank_branch_code')}>
                    <TextField source="bankBranchCode" emptyText="—" />
                </Column>
                <Column source="bankAddress" label={translate('resources.genBankInformation.fields.bank_address')}>
                    <TextField source="bankAddress" emptyText="—" />
                </Column>
            </DataTable>
            <PageTotalFooter />
        </List>
