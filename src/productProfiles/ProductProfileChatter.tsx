...
import {
    postProductProfileChatter,
    type ProductChatterAttachmentPayload,
} from '../api/postProductProfileChatter';
...
/** Odoo-style activity / internal notes for product registration (edit only until saved). */
export function ProductProfileChatter() {
    ...
    const [pendingFiles, setPendingFiles] = React.useState<ProductChatterAttachmentPayload[]>([]);
    const [posting, setPosting] = React.useState(false);
    const [error, setError] = React.useState<string | null>(null);

    React.useEffect(() => {
        const raw = record?.chatterMessages ?? [];
        setMessages(
            raw.map(m => ({
                ...m,
    ...
    const readFileAsPayload = (file: File): Promise<ProductChatterAttachmentPayload> =>
    ...
    const onPickFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const next: ProductChatterAttachmentPayload[] = [];
    ...
    const send = async () => {
        ...
        if (!productId || (!text && pendingFiles.length === 0)) return;
        setPosting(true);
        setError(null);
        try {
            await postProductProfileChatter(productId, text, pendingFiles);
            setBody('');
            setPendingFiles([]);
            refresh();
        } catch (e: unknown) {
    ...
    if (!productId) {
        return (
            <Card variant="outlined" sx={{ borderColor: 'divider', borderStyle: 'dashed' }}>
                <CardContent sx={{ py: 2 }}>
                    <Typography variant="subtitle2" fontWeight={700} gutterBottom>
                        {translate('resources.productProfiles.chatter.title', { _: 'Activity' })}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.8rem' }}>
                        {translate('resources.productProfiles.chatter.after_save', {
                            _: 'Save the product once to log internal notes and attachments here.',
                        })}
                    </Typography>
                </CardContent>
            </Card>
        ...
        >
            <CardContent sx={{ pt: 2, '&:last-child': { pb: 2 } }}>
                ...
                <Typography variant="caption" color="text.secondary" display="block" sx={{ mb: 1 }}>
                    {translate('resources.productProfiles.chatter.hint', {
                        _: 'Internal notes and attachments are stored with this product.',
                    })}
                </Typography>
                ...
                >
                    {translate('resources.productProfiles.chatter.log_note', { _: 'Log note' })}
                </Typography>
                            ...
                            <ListItem alignItems="flex-start" sx={{ px: 0 }}>
                                <ListItemAvatar>
                                    <Avatar sx={{ width: 32, height: 32 }}>
                                        {(m.authorDisplayName || displayName).charAt(0).toUpperCase()}
                                    </Avatar>
                                </ListItemAvatar>
                                <ListItemText
